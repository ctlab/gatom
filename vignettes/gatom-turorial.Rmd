...
```{r}
met.de.meta <- getMetDEMeta(met.de.raw, met.kegg.db)
met.de <- prepareDE(met.de.raw, met.de.meta)
```

Pre-procesing gene data

```{r}
gene.de.meta <- getGeneDEMeta(gene.de.raw, org.Mm.eg.gatom.anno)
gene.de <- prepareDE(gene.de.raw, gene.de.meta)
```

Leaving only top expressed genes

```{r}
gene.de <- gene.de[signalRank <= 12000]
```
