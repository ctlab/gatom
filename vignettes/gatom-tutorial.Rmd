...
If you are using an exact Virgo solver from `mwcsr` package, you may add edge penalty to minimize the size of the solution and remove excessively repeated reactions.
...
To get functional annotation of obtained modules by KEGG and Reactome canonical metabolic pathways, use the `fora()` function from `fgsea` package.
```{r, eval=FALSE}
