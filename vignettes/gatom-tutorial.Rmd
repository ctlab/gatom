...
author: "Anstasiia Gainullina, Alexey Sergushichev, Mariia Emelianova"
date: "August 2023"
output:
...
* You can install **gatom** package via `devtools`:
```{R eval = FALSE}
    #install.packages("devtools")
    # devtools::install_github("ctlab/gatom")
```

 * **gatom** package requires `shinyCyJS` for plotting graphs. It can be installed with `devtools`:
```{R eval = FALSE}
    #install.packages("devtools")
    # devtools::install_github("ctlab/shinyCyJS")
```
...
```{r}
widgetframe::frameWidget(createShinyCyJSWidget(m))
```
...
widgetframe::frameWidget(createShinyCyJSWidget(m.ext))
```
...
widgetframe::frameWidget(createShinyCyJSWidget(m1))
```
...
widgetframe::frameWidget(createShinyCyJSWidget(m2))
```
...
Alternatively, we can save the module to dot format to generate pdf or svg file using `neato` tool from graphviz suite.
...
Full pre-generated combined network (ref. [Networks](#networks) for details) and enzyme annotation can be downloaded from  [http://artyomovlab.wustl.edu/publications/supp_materials/GATOM/](http://artyomovlab.wustl.edu/publications/supp_materials/GATOM/):
...
For better work of the combined network we highly recommend using additional supplementary gene files (ref. [Supplementary Genes](#suppl-genes))
...
```{r message=FALSE, warning=FALSE}
widgetframe::frameWidget(createShinyCyJSWidget(cm))
```
...
KEGG network consists of `network.kegg.rds` & `met.kegg.db.rds` files and is based on [KEGG database](https://www.genome.jp/kegg/kegg1.html). 
...
```{r message=FALSE, warning=FALSE}
widgetframe::frameWidget(createShinyCyJSWidget(km))
```

## Rhea

Rhea network consists of `network.rhea.rds` & `met.rhea.db.rds` files and is based on [Rhea database](https://www.rhea-db.org/). 
Reactions in Rhea have their own IDs, but unlike KEGG, metabolite IDs come from a separate database -- [ChEBI database](https://www.ebi.ac.uk/chebi/)
...
```{r message=FALSE, warning=FALSE}
widgetframe::frameWidget(createShinyCyJSWidget(rm))
```

## Combined network

Combined network comprises not only KEGG and Rhea reactions, but also transport reactions from [BIGG database](http://bigg.ucsd.edu/).
...
For proper work of the lipid network we also need a corresponding supplementary gene file (ref. [Supplementary Genes](#suppl-genes))
...
For lipid network we recommend setting topology to `metabolites` (ref. [Using metabolite-level network](#met-topology)):
...
```{r message=FALSE, warning=FALSE}
widgetframe::frameWidget(createShinyCyJSWidget(lm))
```
...
For combined and lipid networks we provide supplementary files with genes 
...
which requires Java (version >= 11) and CPLEX (version >= 12.7) to be installed. If the requirements are met
...
Sometimes it could make sense to work with metabolite-metabolite topology of the network, not atom-atom one. 
